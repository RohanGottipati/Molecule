...
const PATTERNS = [
  {
    field: "capacity",
    re: /(\d[\d,]*)\s*(?:units?|pieces?|kits?|packs?|hoodies|bottles)?\s*(?:\/|per\s+|a\s+|every\s+)(day|week|month)/gi,
    value: (m) => m[1],
    unit: (m) => `units/${m[2].toLowerCase()}`,
    period: (m) => m[2].toLowerCase(),
  },
  {
    field: "capacity",
    re: /\b(?:capacity|daily_capacity|on_hand|available)\D{0,20}(\d[\d,]*)/gi,
    value: (m) => m[1],
    unit: () => "",
    period: () => "",
  },
  {
    ...
    re: /(\d+(?:\.\d+)?)\s*(hours?|hrs?|h\b|business\s+days?|working\s+days?|days?)/gi,
    value: (m) => m[1],
  ...
  {
    field: "moq",
    re: /\b(?:moq|minimum(?:\s+order)?(?:\s+quantity)?)\D{0,15}(\d+)/gi,
    value: (m) => m[1],
    unit: () => "units",
    period: () => "",
  },
  {
    field: "moq",
    re: /(\d+)\s*(?:unit|pcs?|piece)\s*minimum/gi,
    value: (m) => m[1],
    unit: () => "units",
    period: () => "",
  },
...
function merchantHint(text) {
  ...
  return header
    ? header.replace(/\s*[-–]\s*(PRICE LIST|INVOICE).*/i, "").trim()
    : null;
}
  ...
  for (const artifact of rows) {
    ...
    for (const pattern of PATTERNS) {
      ...
      for (const match of text.matchAll(pattern.re)) {
        ...
        counts.candidates += 1;
        const outcome = !evidencePresent(text, evidence)
          ? "dropped"
          : guard.detected
            ? "blocked"
            : "pending";
        ...
        await db.query(
          ...
          [
            ...
            JSON.stringify({
              ...
              subjectHint: subjectHint(text, match.index ?? 0),
              period: pattern.period(match),
            }),
            pattern.unit(match) || null,
            evidence,
            0.6,
            guard.detected,
            outcome,
            outcome === "blocked"
              ? "Document contains an instruction aimed at the reader"
              : null,
          ],
        );
      }
