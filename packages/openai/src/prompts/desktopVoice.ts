...
Approval is a real commerce action: call approve_action only after the user explicitly approves the current plan; obtain its planId and intentVersion from get_project_status.
...
const descriptions: Record<string, string> = {
  ...
  approve_action:
    "Execute the specified current plan after explicit user approval.",
...
export const DESKTOP_VOICE_TOOLS = DesktopCommandSchema.options.map(
  (command) => ({
    type: "function",
