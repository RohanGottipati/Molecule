...
import {
  rng, pick, chance, intBetween, mojibake, smartQuotes, addBom, crlf, typos,
  truncate, ocrNoise, renderQuantity, renderPrice, renderDate, renderLeadTime, digitTypo,
  INJECTIONS, DIMENSIONS, CURRENCIES,
} from "./chaos.mjs";
...
const VOLUMES = {
  full:  { emails: 120, pricesheets: 40, threads: 60, tickets: 800, wms: 30, portal: 30, briefs: 50, invoices: 60 },
  small: { emails: 12,  pricesheets: 6,  threads: 8,  tickets: 40,  wms: 4,  portal: 4,  briefs: 6,  invoices: 6 },
}[SCALE];
...
const mark = (d) => { dimensionHits[d] = (dimensionHits[d] ?? 0) + 1; };

const daysAgo = (n) => new Date(NOW.getTime() - n * 86400000);
const slug = (s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
const id = (...parts) => createHash("sha256").update(parts.join("|")).digest("hex").slice(0, 12);
...
function stateFact(cap, kind, opts = {}) {
  const chaos = {};
  let text, statedValue, statedUnit, trueValue, trueUnit, expect = "claim", isOutlier = false;

  if (kind === "capacity") {
    trueValue = opts.trueValue ?? cap.capacity;
    trueUnit = `units/${cap.period}`;
    let shown = opts.shownValue ?? trueValue;
    if (opts.drift) { shown = Number(digitTypo(r, shown)); chaos.digit_typo = true; mark("digit_typo"); isOutlier = true; }
    const q = renderQuantity(r, shown, cap.period, cap.unit ?? "units");
    text = q.text;
    statedValue = q.restated ? (q.restated === "week" ? shown * 7 : Math.round(shown / 7)) : shown;
    statedUnit = q.restated ? `units/${q.restated}` : `units/${cap.period}`;
    if (q.restated) { chaos.unit_restated = q.restated; mark("unit_restated"); }
    ...
    if (l.restated) { chaos.unit_restated = l.restated; mark("unit_restated"); }
    if (/^under /.test(l.text)) { chaos.bound = "upper"; }
  } else if (kind === "price") {
    trueValue = opts.trueValue ?? cap.price;
    trueUnit = "CAD";
    const currency = opts.currency ?? (chance(r, 0.25) ? pick(r, ["USD", "GBP", "EUR"]) : "CAD");
    text = renderPrice(r, opts.shownValue ?? trueValue, currency);
    statedValue = Number(((opts.shownValue ?? trueValue) * CURRENCIES[currency]).toFixed(2));
    statedUnit = currency;
    if (currency !== "CAD") { chaos.currency_mixed = currency; mark("currency_mixed"); }
  } else if (kind === "moq") {
    trueValue = opts.trueValue ?? cap.moq;
    trueUnit = "units";
    text = pick(r, [`${trueValue} unit minimum`, `MOQ ${trueValue}`, `minimum order ${trueValue} pcs`]);
  ...
  if (opts.vague) {
    text = pick(r, ["a few hundred", "the usual", "same as last time", "depends on the week", "hard to say right now"]);
    statedValue = null; trueValue = null; expect = "ambiguous";
    chaos.ambiguous = true;
  }

  return { text, field: `${cap.id}.${kind}`, statedValue, statedUnit, trueValue, trueUnit, expect, isOutlier, chaos };
}

/** Applies document-level corruption and records which dimensions were used. */
function corrupt(body, chaos, { allowTruncate = true } = {}) {
  let out = body;
  if (chance(r, 0.18)) { out = mojibake(out); chaos.encoding = "mojibake"; mark("encoding"); }
  else if (chance(r, 0.15)) { out = smartQuotes(out); chaos.encoding = "smart_quotes"; mark("encoding"); }
  if (chance(r, 0.1)) { out = addBom(out); chaos.bom = true; mark("encoding"); }
  if (chance(r, 0.12)) { out = crlf(out); chaos.crlf = true; }
  if (chance(r, 0.22)) { out = typos(r, out); chaos.typos = true; mark("typos"); }
  if (allowTruncate && chance(r, 0.05)) { out = truncate(r, out); chaos.truncated = true; mark("truncated"); }
  return out;
}
...
function maybeInject(body, chaos, path, merchantId) {
  ...
  mark("injection");
  addTruth(path, merchantId, "__injection__", { expect: "ignore", isInjection: true, chaos: { injection: true } });
  return chance(r, 0.5) ? `${body}\n\n${payload}\n` : `${payload}\n\n${body}`;
}

const aliasFor = (m) => {
  if (chance(r, 0.55)) { mark("alias"); return pick(r, m.aliases); }
  return m.name;
};
...
const EN_SUBJECTS = {
  capacity: ["Re: capacity for the onboarding kits", "Updated {noun} numbers", "Quick update", "Production update - {label}"],
  lead_time_hours: ["Re: turnaround times", "Lead time on {label}", "RE: RE: schedule", "Delivery timing"],
  price: ["RE: RE: pricing", "Updated price on {label}", "Quote - {noun}", "Re: costs"],
  moq: ["Minimums", "Re: minimum order quantity", "MOQ for {label}"],
};
const FR_SENTENCE = {
  capacity: (cap, fact) => `Petit suivi sur notre ${cap.noun}. Presentement on est a ${fact.text}.`,
  lead_time_hours: (cap, fact) => `Le delai pour ${cap.label.toLowerCase()} est de ${fact.text}.`,
  price: (cap, fact) => `Le prix unitaire pour ${cap.label.toLowerCase()} est ${fact.text}.`,
...
function genEmail(i) {
  ...
  const kind = pick(r, ["capacity", "capacity", "lead_time_hours", "price", "moq"]);
  ...
  const subject = fr
    ? pick(r, [`Mise a jour - ${cap.noun}`, `Re: commande - delais`, `Capacite de production`])
    : pick(r, EN_SUBJECTS[kind]).replaceAll("{noun}", cap.noun).replaceAll("{label}", cap.label);

  const quoteDate = renderDate(r, daysAgo(intBetween(r, 46, 120)));
  if (quoteDate.ambiguous) { chaos.ambiguous_date = true; mark("ambiguous_date"); }

  const lines = fr
    ? [`Bonjour,`, ``, FR_SENTENCE[kind](cap, fact),
       chance(r, 0.4) ? `Le delai reste ${renderLeadTime(r, cap.leadHours).text}.` : ``,
       ``, `Merci,`, `${contact.name}`, `${contact.role}, ${aliasFor(m)}`]
    : [`Hi,`, ``,
       pick(r, EN_SENTENCES[kind]).replaceAll("{v}", fact.text).replaceAll("{noun}", cap.noun).replaceAll("{label}", cap.label.toLowerCase()),
       chance(r, 0.35) ? `> On ${quoteDate.text} we said ${renderQuantity(r, cap.capacity, cap.period, cap.unit ?? "units").text} - that number is out of date.` : ``,
       chance(r, 0.3) ? `Let me know if you need anything else.` : ``,
       ``, `Thanks,`, `${contact.name}`, `${contact.role} | ${aliasFor(m)}`,
...
function genPriceSheet(i) {
  ...
  if (chance(r, 0.6)) {
    ...
    if (eff.ambiguous) { chaos.ambiguous_date = true; mark("ambiguous_date"); }
    rows.push([`${aliasFor(m)} - PRICE LIST`], [`Effective ${eff.text}`], []);
  }
  rows.push(["SKU", "Item", "Tier", `Unit price (${currency})`, "MOQ", "Lead time"]);

  for (const cap of m.capabilities) {
    const sku = chance(r, 0.4) ? `${cap.id.slice(4, 9).toUpperCase()} ${intBetween(r, 100, 999)}` : `${cap.id.slice(4, 9).toUpperCase()}-${intBetween(r, 100, 999)}`;
    if (sku.includes(" ")) { chaos.sku_drift = true; mark("sku_drift"); }
    ...
    rows.push([sku, cap.label, "1-49", noPrice ? "call for pricing" : price.text.replace(/,/g, ""), moq.statedValue, lead.text]);
    if (chance(r, 0.7)) rows.push(["", "", "50-249", noPrice ? "" : (Number(price.statedValue) * 0.9).toFixed(2), "", ""]);
    if (chance(r, 0.5)) rows.push(["", "", "250+", noPrice ? "" : (Number(price.statedValue) * 0.8).toFixed(2), "", ""]);

    if (noPrice) {
      chaos.missing = true; mark("missing");
      addTruth(path, m.id, price.field, { ...price, statedValue: null, expect: "ambiguous", chaos });
    } else {
  ...
  if (chance(r, 0.35)) rows.push([], ["Note: prices exclude setup fees and are subject to change without notice."]);

  let body = rows.map((row) => row.map((c) => (String(c ?? "").includes(",") ? `"${c}"` : String(c ?? ""))).join(",")).join("\n");
...
function genThread(i) {
  ...
  const stamp = (mins) => `[${day.toISOString().slice(0, 10)}, ${String(9 + Math.floor(mins / 60)).padStart(2, "0")}:${String(mins % 60).padStart(2, "0")}]`;

  // The correction pattern: a number is stated, then revised in the same thread.
  const first = stateFact(cap, "capacity");
  const revisedValue = Math.max(0, Math.round(cap.capacity * pick(r, [0.5, 0.25, 0.75, 0])));
  const revised = stateFact(cap, "capacity", { trueValue: revisedValue, shownValue: revisedValue });
  ...
  const msgs = [
    ...
    chance(r, 0.4) ? `${stamp(intBetween(r, 121, 200))} Molecule Ops: ok noted` : "",
    chance(r, 0.3) ? `${stamp(intBetween(r, 201, 300))} ${contact.name}: <voice note, 0:42>` : "",
  ].filter(Boolean);
...
function genTickets(i) {
  ...
  for (let n = 0; n < count; n++) {
    ...
    const reason = pick(r, [
      "arrived damaged - box crushed", "wrong size hoodie, ordered L got M", "embroidery thread colour is off",
      "engraving misspelled the name", "never arrived, tracking stuck", "missing 3 units from the case",
      "snack pack had nuts, customer is allergic", "late by two days",
    ]);
    const base = { merchant: aliasFor(m), capability: cap.label, opened_at: opened.toISOString(), reason };
    if (schema === "v1") rows.push({ id: `T-${id(path, String(n))}`, ...base, status: pick(r, ["open", "closed", "OPEN", "Closed "]) });
    else if (schema === "v2") rows.push({ ticket_id: `T-${id(path, String(n))}`, merchant_name: base.merchant, capability_name: base.capability, created: base.opened_at, body: { text: reason }, state: pick(r, ["open", "resolved"]) });
    else rows.push({ ticketId: `T-${id(path, String(n))}`, party: { name: base.merchant }, subject: cap.label, createdAt: base.opened_at, description: reason, meta: { state: pick(r, ["open", "resolved"]), channel: pick(r, ["email", "chat", "phone"]) } });
  }
  const payload = schema === "v3" ? { export: { version: 3, tickets: rows } } : rows;
  let body = JSON.stringify(payload, null, 2);
  if (chance(r, 0.08)) { body = truncate(r, body); chaos.truncated = true; mark("truncated"); }
...
function genWms(i) {
  ...
  const path = `wms/${name}`;
  const rows = [["snapshot_date", "location", "item_code", "description", "on_hand", "uom"]];
  for (const cap of ALL_CAPS) {
    if (!chance(r, 0.7)) continue;
    const drift = chance(r, 0.45);
    const shown = drift ? Math.round(cap.capacity * pick(r, [0.5, 0.75, 1.25])) : cap.capacity;
    const itemCode = chance(r, 0.5) ? cap.id.toUpperCase().replaceAll("-", "_") : cap.id;
    if (itemCode !== cap.id) { chaos.sku_drift = true; mark("sku_drift"); }
    rows.push([
      chance(r, 0.3) ? `${day.getUTCDate()}/${day.getUTCMonth() + 1}` : day.toISOString().slice(0, 10),
    ...
    addTruth(path, cap.merchant.id, `${cap.id}.capacity`, {
      statedValue: shown, statedUnit: `units/${cap.period}`,
      trueValue: cap.capacity, trueUnit: `units/${cap.period}`,
      observedAt: day, isOutlier: drift, shouldConflict: drift, chaos,
    });
  }
...
function genPortal(i) {
  ...
  if (stale) { chaos.stale = true; mark("stale"); }
  const name = `${String(i).padStart(3, "0")}-${slug(m.id)}-portal.json`;
  const path = `portal/${name}`;
  const payload = { supplier: aliasFor(m), pulled_at: observed.toISOString(), capabilities: [] };
  for (const cap of m.capabilities) {
    const missing = chance(r, 0.25);
    if (missing) { chaos.missing = true; mark("missing"); }
    const f = stateFact(cap, "capacity");
    payload.capabilities.push({
      code: cap.id,
      name: cap.label,
      daily_capacity: missing ? pick(r, ["N/A", "", null, "unknown"]) : f.statedValue,
...
function genBrief(i) {
  ...
  if (due.ambiguous) { chaos.ambiguous_date = true; mark("ambiguous_date"); }
  const lines = [
    pick(r, [
    ...
    pick(r, ["No leather.", "no leather please", "Nothing with leather in it."]),
    chance(r, 0.6) ? pick(r, ["Hoodie needs our logo embroidered.", "logo on the hoodie (embroidery not print)"]) : "",
    chance(r, 0.5) ? "Bottles should be engraved with each person's name." : "",
    chance(r, 0.5) ? pick(r, ["Snacks must be vegan.", "vegan snacks only"]) : "",
    chance(r, 0.4) ? "Each kit packaged individually." : "",
    // The late correction that the canonical demo depends on.
    chance(r, 0.35) ? pick(r, ["Actually - no polyester either.", "One more thing: no polyester.", "Forgot to say: nothing polyester."]) : "",
  ].filter(Boolean);
  let body = lines.join("\n");
  body = corrupt(body, chaos);
...
function genInvoice(i) {
  ...
  for (const cap of m.capabilities) {
    ...
    const billed = overbilled ? Number((cap.price * pick(r, [1.1, 1.25, 1.5])).toFixed(2)) : cap.price;
    const garbled = chance(r, 0.25);
    const shown = garbled
      ? billed.toFixed(2).replace(/1/g, "l").replace(/0/g, "O").replace(/5/g, "S")
      : billed.toFixed(2);
    if (garbled) chaos.ocr_unparseable = true;
    if (overbilled) { chaos.contradiction = true; mark("contradiction"); }
    total += qty * billed;
    lines.push(
  ...
  lines.push(`------------------------------------------------------------`, `TOTAL (CAD)${statedTotal.toFixed(2).padStart(49)}`);
...
const GENERATORS = { emails: genEmail, pricesheets: genPriceSheet, threads: genThread, tickets: genTickets, wms: genWms, portal: genPortal, briefs: genBrief, invoices: genInvoice };
...
for (const f of files.slice(0, dupeCount)) {
  const name = f.path.split("/").pop().replace(/(\.\w+)$/, "-fwd$1");
  emit(f.type, name, `From: forwarded@molecule.example\nX-Forwarded: true\n\n${f.body}`, { ...f.chaos, duplicate: true }, f.merchantHint);
  mark("duplicate");
  for (const t of truth.filter((t) => t.sourcePath === f.path)) {
    addTruth(`${f.type}/${name}`, t.merchantId, t.field, { ...t, statedValue: t.statedValue, chaos: { ...t.chaos, duplicate: true } });
  }
}
...
await writeFile(join(OUT, "manifest.json"), JSON.stringify({
  batchId: BATCH, seed: SEED, scale: SCALE, generatedAt: NOW.toISOString(),
  artifacts: files.length,
  byType: Object.fromEntries(Object.keys(VOLUMES).map((t) => [t, files.filter((f) => f.type === t).length])),
  truthRows: truth.length,
  expectations: truth.reduce((acc, t) => ({ ...acc, [t.expect]: (acc[t.expect] ?? 0) + 1 }), {}),
  chaosCoverage: dimensionHits,
  aliases: aliasTable().length,
}, null, 2) + "\n", "utf8");

console.log(`batch ${BATCH}: ${files.length} artifacts, ${truth.length} truth rows -> ${join(OUT, "inbox")}`);
console.table(Object.entries(dimensionHits).map(([dimension, hits]) => ({ dimension, hits })));
