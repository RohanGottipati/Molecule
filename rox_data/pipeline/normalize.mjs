...
import {
  SOURCE_AUTHORITY, FIELD_REGISTRY, FX_TO_CAD,
  BUSINESS_DAY_HOURS, CALENDAR_DAY_HOURS,
} from "./config.mjs";
import { shortId, stableJson, bumpStage, emitEvent } from "./db.mjs";

const WORDS = {
  zero: 0, one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9, ten: 10,
  eleven: 11, twelve: 12, twenty: 20, thirty: 30, forty: 40, fifty: 50, sixty: 60, hundred: 100, thousand: 1000,
};
...
export function parseNumber(raw) {
  ...
  if (words.length && words.every((w) => w in WORDS || w === "a" || w === "and")) {
  ...
  s = s.replace(/[$ۣ]/g, "").replace(/\b(cad|usd|gbp|eur)\b/g, "").trim();
  s = s.replace(/^(about|approx\.?|approximately|around|under|over|up to|~)\s*/i, "");
  s = s.replace(/,(?=\d{3}\b)/g, "");           // thousands separators
  s = s.replace(/(\d),(\d)/g, "$1.$2");          // European decimal comma
  s = s.replace(/\s*(units?|pcs?|pieces?|ea|each|kits?|hoodies|bottles|packs?)\b.*$/i, "").trim();
  s = s.replace(/\s*(\/|per\s+)?(day|week|month|hour|hours|h)\b.*$/i, "").trim();
...
export function detectPeriod(unitText, periodHint, rawText) {
  const hay = `${periodHint ?? ""} ${unitText ?? ""} ${rawText ?? ""}`.toLowerCase();
...
export function detectCurrency(unitText, rawText) {
  ...
  for (const code of ["CAD", "USD", "GBP", "EUR"]) if (hay.includes(code)) return code;
...
export function normalizeFact({ fieldKind, value, unit, period, evidence }) {
  ...
  if (!spec) return { ok: false, reason: `Field "${fieldKind}" is outside the registry` };

  const n = parseNumber(value);
  if (n === null) {
    return { ok: false, reason: `Value "${String(value).slice(0, 40)}" is not a number we can read` };
  }
  if (n < 0) return { ok: false, reason: "Negative value is not meaningful for this field" };

  const applied = [];
  if (fieldKind === "capacity") {
    const p = detectPeriod(unit, period, evidence) ?? "day";
    if (!detectPeriod(unit, period, evidence)) applied.push("assumed per-day (no period stated)");
    const perDay = p === "week" ? n / 7 : p === "month" ? n / 30 : n;
    if (p !== "day") applied.push(`${p} -> day`);
    return { ok: true, value: Math.round(perDay * 100) / 100, unit: "units/day", applied, statedPeriod: p };
  }

  if (fieldKind === "lead_time_hours") {
    const hay = `${unit ?? ""} ${evidence ?? ""}`.toLowerCase();
    let hours = n;
    if (/business\s+day|working\s+day/.test(hay)) { hours = n * BUSINESS_DAY_HOURS; applied.push(`business days x${BUSINESS_DAY_HOURS}h`); }
    else if (/\bdays?\b/.test(hay)) { hours = n * CALENDAR_DAY_HOURS; applied.push(`days x${CALENDAR_DAY_HOURS}h`); }
    else if (!/\b(h|hour|hours|hrs?)\b/.test(hay)) applied.push("assumed hours (no unit stated)");
    return { ok: true, value: Math.round(hours * 100) / 100, unit: "hours", applied };
  }

  if (fieldKind === "price") {
    const currency = detectCurrency(unit, evidence) ?? "CAD";
    if (!detectCurrency(unit, evidence)) applied.push("assumed CAD (store currency, none stated)");
    const rate = FX_TO_CAD[currency];
    if (!rate) return { ok: false, reason: `Unknown currency "${currency}"` };
    if (currency !== "CAD") applied.push(`${currency} -> CAD @ ${rate.toFixed(4)}`);
    return { ok: true, value: Math.round(n * rate * 100) / 100, unit: "CAD", applied };
  }
...
export async function normalize(db, { runId, traceId, limit = null }) {
  const counts = { considered: 0, claimed: 0, quarantined: 0, ambiguous: 0, superseded: 0, unlinked: 0 };
  ...
  for (const row of rows) {
    ...
    if (!row.resolved_merchant_id || !row.resolved_field) {
      counts.unlinked += 1;
      await db.query(`update rox_extractions set outcome='dropped', outcome_reason=$2 where extraction_id=$1`,
        [row.extraction_id, "Could not attribute to a merchant and capability"]);
      continue;
    }

    const result = String(stated).trim()
      ? normalizeFact({ fieldKind: row.field, value: stated, unit: row.raw_unit, period: row.raw_value?.period, evidence: row.evidence_text })
      : { ok: false, reason: row.ambiguity ? `Ambiguous: ${row.ambiguity}` : "No value stated" };

    if (!result.ok) {
      const ambiguous = !String(stated).trim();
      if (ambiguous) counts.ambiguous += 1; else counts.quarantined += 1;
      await db.query(
        ...
        [shortId(runId, row.extraction_id, "q"), row.artifact_id, result.reason, runId, row.extraction_id,
         row.resolved_merchant_id, row.resolved_field, JSON.stringify({ value: stated, unit: row.raw_unit })],
      );
      await db.query(`update rox_extractions set outcome='quarantined', outcome_reason=$2 where extraction_id=$1`,
    ...
    const claimId = shortId("claim", row.resolved_merchant_id, row.resolved_field, row.source_reference, stableJson(result.value));
    ...
    const { rowCount: superseded } = await db.query(
      ...
      [row.resolved_merchant_id, row.resolved_field, row.source_reference, claimId, JSON.stringify(result.value)],
    );
    ...
    await db.query(
      ...
      [claimId, row.resolved_merchant_id, row.resolved_field, JSON.stringify(result.value), result.unit,
       row.source_kind, row.source_reference, row.checksum, observedAt, authority,
       Math.min(1, Math.max(0, Number(row.confidence ?? 0.5))), row.evidence_text],
    );
    await db.query(
      ...
      [row.extraction_id, claimId, JSON.stringify(result.value), result.unit, (result.applied ?? []).join("; ")],
    );
    counts.claimed += 1;
  }

  await bumpStage(db, runId, "normalize", counts);
  await emitEvent(db, { traceId, type: "rox.normalize.completed", payload: counts });
  return counts;
}
