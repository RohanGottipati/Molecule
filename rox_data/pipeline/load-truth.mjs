...
const rows = (await readFile(join(corpus, "truth.jsonl"), "utf8"))
  .trim()
  .split("\n")
  .map((l) => JSON.parse(l));
...
const n = await transaction(db, async (client) => {
  await client.query("delete from rox_truth where batch_id = $1", [
    manifest.batchId,
  ]);
  let loaded = 0;
  for (const t of rows) {
    await client.query(
     ...
     on conflict (truth_id) do nothing`,
      [
        ...
        t.field,
        JSON.stringify({
          stated: t.statedValue,
          statedUnit: t.statedUnit,
          true: t.trueValue,
          trueUnit: t.trueUnit,
          expect: t.expect,
