...
async function completed(kind, pass = null) {
  return (
    (
      await q(
        "select 1 from bulk_load_runs where kind=$1 and coalesce(pass,-1)=coalesce($2,-1) and status='completed'",
        [kind, pass],
      )
    ).rowCount > 0
  );
...
async function startRun(kind, pass = null, detail = {}) {
  return (
    await q(
      "insert into bulk_load_runs(kind,pass,detail) values($1,$2,$3) returning id",
      [kind, pass, detail],
    )
  ).rows[0].id;
}
const finishRun = (id, rows, status = "completed") =>
  q(
    "update bulk_load_runs set rows_loaded=$2, status=$3, finished_at=now() where id=$1",
    [id, rows, status],
  );
const dbSizeGb = async () =>
  Number(
    (
      await q(
        "select pg_database_size(current_database())::float8 / 1073741824 as gb",
      )
    ).rows[0].gb,
  );
...
async function loadUci(file) {
  if (await completed("uci_pass0", 0)) {
    log("uci pass 0 already loaded (skip)");
    return true;
  }
  const prev = (
    await q(
      "select id from bulk_load_runs where kind='uci_pass0' and status in ('running','failed') order by id desc limit 1",
    )
  ).rows[0];
  let run,
    resumeFrom = -1;
  if (prev) {
    run = prev.id;
    await q("update bulk_load_runs set status='running' where id=$1", [run]);
    resumeFrom = Number(
      (
        await q(
          "select coalesce(max(src_row), -1) m from bulk_order_lines where pass=0",
        )
      ).rows[0].m,
    );
  ...
  try {
    for await (const r of csvRows(file)) {
      ...
      if (outOfTime()) {
        await flush();
        await q(
          "update bulk_load_runs set rows_loaded=(select count(*) from bulk_order_lines where pass=0) where id=$1",
          [run],
        );
        log(
          "time budget reached, loaded so far",
          total,
      ...
      cols.ts.push(`${ts}+00`);
      cols.row.push(Number(srcRow));
      cols.inv.push(invoice);
      cols.sku.push(code);
      cols.desc.push(desc);
      cols.qty.push(Number(qty));
      cols.price.push(price);
      cols.cust.push(cust || null);
      cols.country.push(country);
      cols.cancel.push(invoice.startsWith("C"));
    ...
    const all = Number(
      (await q("select count(*) n from bulk_order_lines where pass=0")).rows[0]
        .n,
    );
    ...
    await finishRun(run, total, "failed");
    throw e;
  }
}
...
async function loadOff(file) {
  if (await completed("off_products")) {
    log("off products already loaded (skip)");
    return true;
  }
  await q(
    "delete from bulk_load_runs where kind='off_products' and status<>'completed'",
  );
  const run = await startRun("off_products", null, {
    file: file.split("/").pop(),
  });
  let header = null,
    total = 0,
  ...
  try {
    for await (const r of csvRows(file)) {
      ...
      if (batch.length >= 5000) {
        await flush();
        if (total % 25000 === 0) log("off products loaded", total);
      }
    }
    await flush();
...
async function replay({ target, maxGb, maxPass }) {
  const bounds = (
    await q(
      "select date_trunc('month', min(invoice_ts)) lo, date_trunc('month', max(invoice_ts)) + interval '1 month' hi from bulk_order_lines where pass=0",
    )
  ).rows[0];
  if (!bounds.lo) throw new Error("pass 0 is empty: load --uci first");
  for (let k = 1; k <= maxPass; k++) {
    const have = Number(
      (await q("select count(*)::bigint n from bulk_order_lines")).rows[0].n,
    );
    if (have >= target) {
      log(`target reached: ${have} rows >= ${target}`);
      return true;
    }
    ...
    if (gb >= maxGb) {
      log(
        `size guard: database is ${gb.toFixed(2)} GB >= ${maxGb} GB, stopping`,
      );
      return true;
    }
    ...
    const prev = (
      await q(
        "select id from bulk_load_runs where kind='replay' and pass=$1 and status in ('running','failed') order by id desc limit 1",
        [k],
      )
    ).rows[0];
    const run = prev
      ? prev.id
      : await startRun("replay", k, {
          shift,
          price_drift: `1.03^${years.toFixed(2)}`,
        });
    if (prev)
      await q("update bulk_load_runs set status='running' where id=$1", [run]);
    let rows = 0;
    try {
      for (
        let m = new Date(bounds.lo);
        m < new Date(bounds.hi);
        m = new Date(Date.UTC(m.getUTCFullYear(), m.getUTCMonth() + 1, 1))
      ) {
        const next = new Date(
          Date.UTC(m.getUTCFullYear(), m.getUTCMonth() + 1, 1),
        );
        // month batches are atomic: if this source month is already present in the pass, skip it (resume)
        const done = await q(
          "select 1 from bulk_order_lines where pass=$1 and invoice_ts >= $2::timestamptz + $3::interval and invoice_ts < $4::timestamptz + $3::interval limit 1",
          [k, m.toISOString(), shift, next.toISOString()],
        );
        if (done.rowCount) continue;
        if (outOfTime()) {
          await q(
            "update bulk_load_runs set rows_loaded=(select count(*) from bulk_order_lines where pass=$1) where id=$2",
            [k, run],
          );
          log(
            `time budget reached during replay pass ${k} - re-run the same command to resume`,
          );
          return false;
        }
        const r = await q(
          ...
          [
            ...
            m.toISOString(),
            next.toISOString(),
            SCALE[(k - 1) % SCALE.length],
            KEEP[(k - 1) % KEEP.length],
          ],
        );
        rows += r.rowCount;
      }
      const all = Number(
        (await q("select count(*) n from bulk_order_lines where pass=$1", [k]))
          .rows[0].n,
      );
      await finishRun(run, all);
      log(
        `replay pass ${k}: ${all} rows (shift ${shift}); db ${(await dbSizeGb()).toFixed(2)} GB`,
      );
    } catch (e) {
      await finishRun(run, rows, "failed");
      throw e;
    }
  }
  return true;
...
async function compressOld() {
  const chunks = (
    await q(`select c::text as chunk from show_chunks('bulk_order_lines', older_than => interval '60 days') c
    where not exists (select 1 from timescaledb_information.chunks i where i.chunk_schema || '.' || i.chunk_name = c::text and i.is_compressed)`)
  ).rows;
  let n = 0;
  for (const { chunk } of chunks) {
    if (outOfTime()) {
      log(
        `time budget reached: compressed ${n}/${chunks.length} chunks - re-run --compress to continue`,
      );
      return false;
    }
    await q("select compress_chunk($1::regclass, if_not_compressed => true)", [
      chunk,
    ]);
    n++;
  }
...
async function aggregates() {
  ...
  await q(`create materialized view if not exists bulk_product_monthly with (timescaledb.continuous) as
    select time_bucket('1 month', invoice_ts) as month, sku,
      ...
      count(*) as lines
    from bulk_order_lines group by 1, 2 with no data`);
  for (const v of ["bulk_sales_daily", "bulk_product_monthly"]) {
...
async function stats() {
  ...
  const r = (
    await q(`select (select count(*) from bulk_order_lines) lines, (select count(*) from bulk_order_lines where pass=0) real_lines,
    (select count(*) from bulk_products) products, (select min(day)::date from bulk_sales_daily) first_day, (select max(day)::date from bulk_sales_daily) last_day`)
  ).rows[0];
...
try {
  ...
  if (finished && args.replay)
    finished = await replay({
      target: num(args["target-rows"], 5_000_000),
      maxGb: num(args["max-gb"], 8),
      maxPass: num(args["max-pass"], 8),
    });
  if (args.compress) finished = (await compressOld()) && finished;
  if (finished && (args.aggregates || args.uci || args.replay))
    await aggregates();
  await stats();
