...
export class DatabaseCapacityStore implements CapacityStore {
  ...
  async getAvailableCapacity(
    ...
    const result = await getPool().query<{ available: string }>(
      `select greatest(0,(c.capability_json->'capacity'->>'available')::numeric
        - coalesce((select sum(quantity) from reservations r where r.capability_id=c.capability_id
          and r.status='active' and r.expires_at>now()),0))::text as available
  ...
  async reserve(input: ReserveCapacityInput): Promise<CapacityReservation> {
    ...
    const result = await action(input.actionKey, input, async (client) => {
      ...
      if (!capability.rowCount) throw new Error("Unknown merchant capability");
      ...
      const reserved = await client.query<{ total: string }>(
        `select coalesce(sum(quantity),0)::text as total from reservations
         where capability_id=$1 and status='active' and expires_at>now()`,
        [input.capabilityId],
      );
      const available =
        Number(capability.rows[0]?.available ?? 0) -
