    ...
    nodes: [
      {
        ...
        completesAt: "2026-09-19T13:00:00Z",
        resourceRefs: [
          {
            resourceId: `${id}:${kind}`,
            kind,
            unit: "units",
            unitsPerItem: 1,
            available: kind === "inventory" ? 3 : 2,
            ...(kind === "processing" ? { periodMinutes: 60 } : {}),
            observedAt,
            sourceReference: "fixture:reservation",
          },
        ],
      },
    ],
...
describe.skipIf(!database)(
  "transactional shared inventory and scheduled machine reservations",
  () => {
    beforeAll(async () => {
    ...
    it("allows exactly one concurrent stock commit and preserves idempotent retries", async () => {
      ...
      const results = await Promise.allSettled([
        reserveCatalogPlan(a, id, `${id}:a`),
        reserveCatalogPlan(b, id, `${id}:b`),
      ]);
      ...
      await expect(
        reserveCatalogPlan({ ...winner, planId: "changed" }, id, key),
    ...
    it("rejects overlapping machine intervals but admits an adjacent interval", async () => {
      const a = plan("machine-a", "processing"),
        b = plan("machine-b", "processing");
      await reserveCatalogPlan(a, id, `${id}:machine-a`);
      await expect(
        reserveCatalogPlan(b, id, `${id}:machine-b`),
      ).rejects.toThrow("PROCESSING_INTERVAL_RESERVED");
