...
const QUERIES = {
  revenue_by_year: [
    `Revenue and refunds by year (continuous aggregate: pre-computed daily rollup over ~5M lines)`,
  ...
  top_products: [
    `Top 10 products by lifetime revenue (monthly product aggregate joined to the product catalog)`,
  ...
  seasonality: [
    `Which month of the year sells the most (window over the daily aggregate)`,
  ...
  rfm: [
    `RFM customer segments on the last 2 years of orders (recency / frequency / monetary, quintiles via NTILE)`,
    `with c as (
  ...
  basket_pairs: [
    `Products most often bought together (market-basket self-join, one year of real orders)`,
  ...
  cohorts: [
    `Customer cohort retention: share of each first-purchase cohort still ordering N months later`,
  ...
  food_nutrition: [
    `Open Food Facts catalog: average nutrition by Nutri-Score (real product data)`,
