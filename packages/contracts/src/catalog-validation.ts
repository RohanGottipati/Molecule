...
export function validateCatalogJsonl(jsonl: string): CatalogValidationReport {
  ...
  if (!manifest) {
    report.errors.push("MISSING_MANIFEST");
    return report;
  }
  ...
  if (new Set(manifest.categories).size !== manifest.categories.length)
    report.errors.push("DUPLICATE_CATEGORY");
  for (const record of report.records) {
    if (byId.has(record.id)) report.errors.push(`DUPLICATE_ID ${record.id}`);
    byId.set(record.id, record);
    if (Date.parse(record.evidence.observedAt) > Date.parse(manifest.createdAt))
      report.errors.push(`FUTURE_EVIDENCE ${record.id}`);
    if (record.recordType === "merchant" && record.shopDomain) {
      if (stores.has(record.shopDomain))
        report.errors.push(`DUPLICATE_STORE ${record.shopDomain}`);
    ...
    if (record.recordType === "variant") {
      ...
      if (record.shopify) {
        ...
        if (variantMappings.has(mapping))
          report.errors.push(`DUPLICATE_VARIANT_MAPPING ${record.id}`);
    ...
    if (record.recordType === "resource" && record.inventoryItemGid) {
      ...
      if (inventoryMappings.has(key))
        report.errors.push(`DUPLICATE_RESOURCE_MAPPING ${record.id}`);
      inventoryMappings.add(key);
    }
    if (
      (record.recordType === "product" || record.recordType === "recipe") &&
      !manifest.categories.includes(record.category)
    )
  ...
  for (const [kind, count] of Object.entries(manifest.recordCounts)) {
    if (
      report.records.filter((record) => record.recordType === kind).length !==
      count
    )
  ...
  function reference(
    ...
    if (
      ...
      target.merchantId !== record.merchantId
    )
      report.errors.push(`CROSS_MERCHANT_REFERENCE ${record.id}.${field}`);
    return target;
  }
  for (const record of report.records) {
    if ("merchantId" in record)
      reference(record, "merchantId", record.merchantId, "merchant");
    if (record.recordType === "variant") {
      reference(record, "productId", record.productId, "product");
      if (
        record.material.status === "known" &&
        (typeof record.material.value !== "string" ||
          !record.material.value.trim())
      )
        report.errors.push(`INVALID_MATERIAL ${record.id}`);
    }
    if (record.recordType === "fact")
      reference(record, "subjectId", record.subjectId, "binding");
    ...
    if (variant?.recordType === "variant") {
      ...
      if (
        ...
        !variant.supportedOperations.includes(family.operation)
      )
    ...
    for (const [field, id] of Object.entries(record.factIds)) {
      ...
      if (fact.subjectId !== record.id || fact.field !== field)
        report.errors.push(`FACT_SCOPE_MISMATCH ${record.id}.${field}`);
      if (fact.assertion.status !== "known")
        reasons.push(`${field}:${fact.assertion.status}`);
    }
    if (
      new Set(record.resources.map((r) => r.resourceId)).size !==
    ...
    for (const ref of record.resources) {
      ...
      if (resource?.recordType === "resource") {
        if (resource.availability.status !== "known")
          reasons.push(`${resource.id}:${resource.availability.status}`);
        else if (resource.availability.value === 0)
          reasons.push(`${resource.id}:unavailable`);
        if (variant?.recordType === "variant" && resource.unit !== variant.unit)
          reasons.push(`${resource.id}:unit_mismatch`);
      }
    }
    report.exclusions.push({ bindingId: record.id, reasons });
  }
  report.coverage = manifest.categories.map((category) => {
    const products = report.records.filter(
      (r) => r.recordType === "product" && r.category === category,
    );
    const ids = new Set(products.map((p) => p.id));
    const variants = new Set(
      report.records
        .filter((r) => r.recordType === "variant" && ids.has(r.productId))
        .map((r) => r.id),
    );
    const ready = new Set(
      report.exclusions
    ...
    return {
      ...
      recipes: report.records.filter(
        (r) => r.recordType === "recipe" && r.category === category,
      ).length,
      readyBindings: report.records.filter(
        (r) =>
          r.recordType === "binding" &&
          variants.has(r.variantId) &&
          ready.has(r.id),
      ).length,
    };
  });
