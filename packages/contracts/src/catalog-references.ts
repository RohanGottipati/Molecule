...
export const SelectedCatalogItemSchema = z.strictObject({
  ...
  variantGid: z.string().regex(/^gid:\/\/shopify\/ProductVariant\/\d+$/).optional(),
});

export const CatalogResourceReferenceSchema = z.strictObject({
  ...
  occupiedIntervals: z.array(z.strictObject({ startsAt: z.iso.datetime(), completesAt: z.iso.datetime() }).refine(interval => Date.parse(interval.completesAt) > Date.parse(interval.startsAt))).optional(),
}).superRefine((resource, context) => {
  if ((resource.kind === "processing") !== (resource.periodMinutes !== undefined))
    context.addIssue({ code: "custom", message: "Only processing resources require periodMinutes" });
});
