...
export async function syncCatalogInventory(
  transportFor: (domain: string) => Pick<ShopifyTransport, "getInventory"> | undefined,
  traceId: string,
): Promise<{ observed: number; unavailable: string[] }> {
  const mappings = await getPool().query<{ resource_id: string; domain: string; item: string; location: string }>(`
  ...
  const snapshots = new Map<string, Awaited<ReturnType<ShopifyTransport["getInventory"]>>>();
  for (const mapping of mappings.rows) {
    const transport = transportFor(mapping.domain);
    if (!transport) { unavailable.push(mapping.resource_id); continue; }
    const key = `${mapping.domain}/${mapping.item}`;
    let snapshot = snapshots.get(key);
    if (!snapshot) { snapshot = await transport.getInventory(mapping.item); snapshots.set(key, snapshot); }
    const level = snapshot.inventoryItem?.inventoryLevels.nodes.find(row => row.location.id === mapping.location);
    const available = level?.quantities.find(q => q.name === "available")?.quantity;
    if (!snapshot.inventoryItem?.tracked || !level?.updatedAt || available === undefined) {
      ...
      await transaction(async client => {
        const changed = await client.query("update catalog_resource_state set status='unknown' where resource_id=$1 and status<>'unknown' returning merchant_id", [mapping.resource_id]);
        ...
        await client.query(`insert into catalog_recovery_requests(order_id,resource_id,observation_key)
          ...
          on conflict(order_id,resource_id) do update set observation_key=excluded.observation_key,status='pending',updated_at=now()`, [mapping.resource_id,key]);
        await persistEvent({ eventId: effectId(key), traceId, merchantId: changed.rows[0].merchant_id, eventType: "catalog.resource.unavailable", source: "shopify", severity: "WARN", ts: new Date().toISOString(), payload: { resourceId: mapping.resource_id, reason: "missing_tracked_location_or_observation" } }, client);
      });
      continue;
    }
    await observeCatalogInventory({ shop: mapping.domain, inventoryItemId: mapping.item, locationId: mapping.location, available, observedAt: level.updatedAt, traceId });
    observed++;
  }
