...
const counts = await transaction(db, async (c) => {
  ...
  const del = async (label, sql, params) => {
    n[label] = (await c.query(sql, params)).rowCount;
  };
  ...
  await del(
    "reviews",
    `delete from rox_review_queue where run_id in
    (select run_id from rox_ingest_runs where batch_id = $1)`,
    [batchId],
  );
  await del(
    "quarantined",
    `delete from quarantined_claims where artifact_id in
    (select artifact_id from raw_artifacts where batch_id = $1)`,
    [batchId],
  );
  await del("artifacts", `delete from raw_artifacts where batch_id = $1`, [
    batchId,
  ]); // cascades to extractions
  await del(
    "resolutions",
    `delete from canonical_resolutions where winning_claim_id in
    (select claim_id from canonical_claims where source_reference like $1)`,
    [prefix],
  );
  await del(
    "conflicts",
    `delete from claim_conflicts where exists (
    select 1 from canonical_claims c where c.source_reference like $1 and c.claim_id = any(claim_ids))`,
    [prefix],
  );
  await del(
    "claims",
    `delete from canonical_claims where source_reference like $1`,
    [prefix],
  );
  await del("runs", `delete from rox_ingest_runs where batch_id = $1`, [
    batchId,
  ]);
  if (!args["keep-truth"])
    await del("truth", `delete from rox_truth where batch_id = $1`, [batchId]);
  return n;
});
