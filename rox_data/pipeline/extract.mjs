...
export function detectInjection(text) {
  const hits = INJECTION_PATTERNS.filter((p) => p.test(text)).map((p) =>
    p.source.slice(0, 48),
  );
...
async function extractOne(client, db, runId, artifact) {
  ...
  const budget = await meter(db, runId, {
    stage: "extract",
    model: MODELS.extract,
    usage,
  ...
  try {
    ...
    throw new Error(
      `Model returned unparseable JSON for ${artifact.source_path}`,
    );
  }
  ...
  async function worker() {
    while (queue.length && !aborted) {
      ...
      try {
        ...
        if (dry) continue;
        const { parsed, budget } = await extractOne(
          client,
          db,
          runId,
          artifact,
        ...
        if (budget.spent >= budget.budget) {
          aborted = new BudgetExceeded(budget.spent, budget.budget);
          break;
        }
        ...
        for (const c of parsed.candidates ?? []) {
          ...
          await db.query(
            ...
            [
              ...
              c.ambiguity || null,
              parsed.documentDate &&
              !Number.isNaN(Date.parse(parsed.documentDate))
        ...
        if (injection) {
          await db.query(
            ...
            [
              shortId(runId, artifact.artifact_id, "injection"),
              runId,
              parsed.merchantHint || null,
              {
                ...
                patterns: guard.hits,
                modelReported: parsed.injectionDetected === true,
                note: parsed.injectionNote ?? null,
              },
              {
                action: "quarantine_document",
                reason: "Instruction-like text in supplier content",
              },
            ],
          );
          await emitEvent(db, {
            traceId,
            type: "rox.injection.blocked",
            severity: "WARN",
            payload: {
              artifactId: artifact.artifact_id,
              sourcePath: artifact.source_path,
