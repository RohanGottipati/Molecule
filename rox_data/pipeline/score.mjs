...
const runId = String(
  args.run ??
    (
      await db.query(
        `select run_id from rox_ingest_runs order by started_at desc limit 1`,
      )
    ).rows[0]?.run_id,
);
const {
...
const { rows: seen } = await db.query(
  `select distinct a.source_path from raw_artifacts a
     join rox_extractions x using (artifact_id) where x.run_id = $1`,
  [runId],
);
...
const { rows: extractions } = await db.query(
  `select x.*, a.source_path from rox_extractions x join raw_artifacts a using (artifact_id) where x.run_id = $1`,
  [runId],
);
const { rows: quarantined } = await db.query(
  `select q.*, a.source_path from quarantined_claims q join raw_artifacts a using (artifact_id) where q.run_id = $1`,
  [runId],
);
const { rows: resolutions } = await db.query(
  `select * from canonical_resolutions`,
);
...
for (const t of truth) {
  ...
  if (t.expect === "quarantine") {
    const q = quarantined.find(
      (x) => x.source_path === t.source_path && kindOf(x.field ?? "") === kind,
    );
  ...
  if (match.x.outcome !== "claimed") m.normalization.missing += 1;
  ...
  } else if (close(Number(match.x.normalized_value), trueValue))
    ...
    failures.push({
      ...
      path: t.source_path,
      field: t.field,
      detail: `normalized to ${match.x.normalized_value} ${match.x.normalized_unit}, truth is ${trueValue} (stated ${t.truth_value} ${t.true_unit})`,
    });
  }
  ...
  if (t.is_outlier) {
    ...
    const accepted =
      res?.status === "resolved" &&
      close(Number(res.value), Number(match.parsed));
    if (accepted) {
...
const truthKeys = new Set(
  truth.map((t) => `${t.source_path}|${kindOf(t.field)}`),
);
for (const x of extractions) {
