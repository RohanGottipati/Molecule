...
// generate-catalog first) instead of 4 hardcoded demo capabilities.
//
// Inserts in batches so you get progress output instead of one giant
// silent INSERT. Safe to run more than once -- it only *adds* rows.
//
// Run (after generate-catalog):
//   pnpm --filter @molecule/db bulk-seed
//
// Default is 3,000,000 rows split across the three hypertables. Pass a
...
async function insertFulfillmentBatch(
  ...
  await client.query(
    ...
    select
      now() - (random() * interval '90 days') - ((gs + $5) * interval '1 microsecond'),
      ($1::text[])[idx],
      ($2::text[])[idx],
      promised,
...
async function insertMarketBatch(
  ...
  await client.query(
    ...
    from generate_series(1, $3) gs,
         lateral (select gs as _forced, floor(random() * $4 + 1)::int as idx) i;
  `,
    [merchantIds, capabilityIds, rows, pairCount, offset],
  );
}
...
async function insertNetworkBatch(
  ...
  await client.query(
    ...
    select
      now() - (random() * interval '90 days') - ((gs + $4) * interval '1 microsecond'),
    ...
    from generate_series(1, $2) gs,
         lateral (select gs as _forced, floor(random() * $3 + 1)::int as idx) i;
  `,
